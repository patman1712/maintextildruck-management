    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <p className="text-sm text-gray-600 mb-4">
            Definieren Sie hier globale Attribute wie Größen oder Farben, die in den Shops verwendet werden können.
        </p>

        {editingVariable && (
            <div className="mb-6 bg-slate-50 p-4 rounded border border-blue-200">
                <h3 className="font-bold text-slate-800 mb-4">{editingVariable.id ? 'Variable bearbeiten' : 'Neue Variable'}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="md:col-span-2">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                            {shops.map(shop => (
                                <label key={shop.id} className="flex items-center space-x-2 bg-white p-2 rounded border border-gray-200 cursor-pointer hover:bg-gray-50">
                                    <input 
                                        type="checkbox" 
        ...
        <div className="space-y-2">
            {variables.length === 0 ? (
                <p className="text-center text-gray-400 py-4">Noch keine Variablen angelegt.</p>
            ) : (
                variables.map(v => (
                    <div key={v.id} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded hover:bg-white transition-colors">
                        <div>
                            <div className="flex items-center space-x-2">
                                <span className="font-bold text-slate-800">{v.name}</span>
                                <span className="text-xs px-2 py-0.5 rounded bg-slate-200 text-slate-600 uppercase">{v.type === 'size' ? 'Größe' : 'Farbe'}</span>
                            </div>
                            <p className="text-sm text-gray-600 mt-1 truncate max-w-md">{v.values}</p>
                            <div className="flex items-center mt-2 space-x-2">
                                <span className="text-xs text-gray-400">Shops:</span>
                                {(v.assigned_shop_ids && v.assigned_shop_ids.length > 0) ? (
                                    <div className="flex -space-x-1">
                                        {v.assigned_shop_ids.map((sid: string) => {
                                            const s = shops.find(shop => shop.id === sid);
                                            return s ? (
                                                <span key={sid} className="w-4 h-4 rounded-full bg-blue-100 border border-white flex items-center justify-center text-[8px] text-blue-800" title={s.name}>
                                                    {s.name.substring(0,1)}
                                                </span>
                                            ) : null;
                                        })}
                                        <span className="text-xs text-gray-500 ml-2">({v.assigned_shop_ids.length})</span>
                                    </div>
                                ) : (
                                    <span className="text-xs text-gray-400 italic">Keine Zuordnung</span>
                                )}
                            </div>
                        </div>
                        <div className="flex items-center space-x-2">
                            <button 
                                onClick={() => setEditingVariable({...v, shop_ids: v.assigned_shop_ids || []})}
